...
impl EventSet {
    /// Checks if `self` contains only the bits in `other_set`.
    pub fn contains_only(self, other_set: EventSet) -> bool {
        (self - other_set).is_empty()
    }
}
...
mod tests {
    ...
    fn test_event_set() {
        ...
        assert!(event_set.contains_only(EventSet::IN | EventSet::ERROR));
        assert!(!event_set.contains_only(EventSet::IN));
        assert!(!event_set.contains_only(EventSet::ERROR | EventSet::OUT));
    }
