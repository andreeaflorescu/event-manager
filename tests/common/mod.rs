...
//
// For some reason clippy & check think that the public methods in this file are not used
// (even though they are). Let's allow dead code so that we don't get warnings all the time.
// This is a bit sad, hopefully it gets fixed in more recent rust versions.
#![allow(dead_code)]
